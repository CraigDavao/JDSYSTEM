...
/* Query for Baby Boys only */
$sql  = "SELECT id, name, price, image, category, created_at
         FROM products
         WHERE is_active = 1 AND category = 'baby-boys'
...
$countSql = "SELECT COUNT(*) AS c FROM products WHERE is_active=1 AND category='babyboys'";
...
<head>
  ...
  <title>New Baby Boys</title>
</head>
<body>

  <div class="new-header">
    <h1 class="new-title">New Baby Boys</h1>
  </div>
  ...
  <?php if ($totalPages > 1): ?>
    <div class="pager">
      <?php for ($i=1; $i <= $totalPages; $i++): ?>
        ...
        <?php else: ?>
          <a href="<?= SITE_URL ?>pages/babyboys.php?page=<?= $i ?>"><?= $i ?></a>
        <?php endif; ?>
      <?php endfor; ?>
    </div>
  <?php endif; ?>

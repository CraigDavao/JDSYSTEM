...
if ($subcategory && !in_array($subcategory, $allowedSub)) $subcategory = null;
...
// 🟢 Fetch products using reusable backend
...
<head>
  ...
  <title>
    Kid<?= $gender ? ' — ' . ucfirst($gender) : '' ?><?= $subcategory ? ' — ' . str_replace('-', ' ', ucfirst($subcategory)) : '' ?>
  </title>
    ...
    <?php if (!empty($processed_products)): ?>
      <?php foreach ($processed_products as $product): ?>
        <?php
          ...
          // 🩷 Use product_image (Base64 from get-products.php)
  ...
  <?php if ($totalPages > 1): ?>
    <div class="pager">
      <?php for ($i = 1; $i <= $totalPages; $i++): ?>
        <?php 
          $queryString = '?page=' . $i;
          if ($gender) $queryString .= '&gender=' . urlencode($gender);
          if ($subcategory) $queryString .= '&subcategory=' . urlencode($subcategory);
        ?>
        <?php if ($i === $page): ?>
          <span class="current"><?= $i ?></span>
        <?php else: ?>
          <a href="<?= SITE_URL ?>pages/kid.php<?= $queryString ?>"><?= $i ?></a>
        <?php endif; ?>
      <?php endfor; ?>
    </div>
  <?php endif; ?>

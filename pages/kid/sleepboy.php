...
<head>
  ...
  <title>Boys <?= ucfirst(str_replace('-', ' ', $subcategory)) ?></title>
</head>
<body>

  <div class="new-header">
    <h1 class="new-title">Boys <?= ucfirst(str_replace('-', ' ', $subcategory)) ?></h1>
  </div>
  ...
  <div class="product-grid">
    <?php if (!empty($processed_products)): ?>
      ...
      <?php endforeach; ?>
    <?php else: ?>
      <p style="grid-column:1/-1;opacity:.7;">No boys <?= htmlspecialchars($subcategory) ?> found.</p>
    <?php endif; ?>
  </div>
  ...
  <?php if ($totalPages > 1): ?>
    <div class="pager">
      <?php for ($i = 1; $i <= $totalPages; $i++): ?>
        ...
        <?php else: ?>
          <a href="<?= SITE_URL ?>pages/<?= $category ?>.php?page=<?= $i ?>&subcategory=<?= urlencode($subcategory) ?>"><?= $i ?></a>
        <?php endif; ?>
      <?php endfor; ?>
    </div>
  <?php endif; ?>

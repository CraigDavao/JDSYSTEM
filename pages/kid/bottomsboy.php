  ...
  <div class="product-grid">
    <?php if ($products->num_rows): ?>
      <?php while ($p = $products->fetch_assoc()): ?>
        <?php $hasSale = isset($p['sale_price']) && $p['sale_price'] > 0 && $p['sale_price'] < $p['price']; ?>
      ...
      <?php endwhile; ?>
    <?php else: ?>
      <p style="grid-column:1/-1;opacity:.7;">No boys bottoms found.</p>
    <?php endif; ?>
  </div>
  ...
  <?php if ($totalPages > 1): ?>
    <div class="pager">
      <?php for ($i = 1; $i <= $totalPages; $i++): ?>
        ...
        <?php else: ?>
          <a href="<?= SITE_URL ?>pages/kids/bottomsboy.php?page=<?= $i ?>"><?= $i ?></a>
        <?php endif; ?>
      <?php endfor; ?>
    </div>
  <?php endif; ?>

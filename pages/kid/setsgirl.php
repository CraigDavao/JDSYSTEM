...
$stmt->execute();
$products = $stmt->get_result(); // Changed variable name to match usage
...
<body>
  <div class="new-header">
    <h1 class="new-title">Girls Sets</h1>
  </div>
  ...
  <div class="product-grid">
    <?php if ($products->num_rows): ?>
      ...
      <?php endwhile; ?>
    <?php else: ?>
      <p style="grid-column:1/-1; opacity:.7;">No girls sets found.</p>
    <?php endif; ?>
  </div>
  ...
  <?php if ($totalPages > 1): ?>
    <div class="pager">
      <?php for ($i = 1; $i <= $totalPages; $i++): ?>
        ...
        <?php else: ?>
          <a href="<?= SITE_URL ?>pages/setsgirl.php?page=<?= $i ?>"><?= $i ?></a>
        <?php endif; ?>
      <?php endfor; ?>
    </div>
  <?php endif; ?>

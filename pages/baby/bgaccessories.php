...
// Fixed filters: Baby → Girls → Accessories
$categoryGroup = 'baby';
$gender        = 'girls';
...
// Query products including sale info
...
<head>
  <meta charset="utf-8">
  <title>Baby Girls — Accessories</title>
  <link rel="stylesheet" href="<?= SITE_URL ?>css/new.css?v=<?= time() ?>">
</head>
<body>
  <div class="new-header">
    <h1 class="new-title">Baby Girls Accessories</h1>
  </div>

  <div class="product-grid">
    <?php if ($result->num_rows): while ($p = $result->fetch_assoc()): ?>
      <?php
        $isOnSale = !empty($p['sale_price']) && $p['sale_price'] > 0;
      ?>
    ...
    <?php endwhile; else: ?>
      <p style="grid-column:1/-1; opacity:.7;">No baby girls accessories found.</p>
    <?php endif; ?>
  </div>
  ...
  <?php if ($totalPages > 1): ?>
    <div class="pager">
      <?php for ($i=1; $i <= $totalPages; $i++): ?>
        ...
        <?php else: ?>
          <a href="<?= SITE_URL ?>pages/baby/bgaccessories.php?page=<?= $i ?>"><?= $i ?></a>
        <?php endif; ?>
      <?php endfor; ?>
    </div>
  <?php endif; ?>

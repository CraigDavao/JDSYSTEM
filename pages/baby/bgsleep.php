...
// Fixed filters: Baby → Girls → Sleepwear & Underwear
$categoryGroup = 'baby';
$gender        = 'girls';
...
$stmt->execute();
$products = $stmt->get_result(); // Changed variable name
...
<head>
  <meta charset="utf-8">
  <title>Baby Girls — Sleepwear & Underwear</title>
...
<body>
  <div class="new-header">
    <h1 class="new-title">Baby Girls Sleepwear & Underwear</h1>
  </div>
  ...
  <div class="product-grid">
    <?php if ($products->num_rows): ?>
      ...
      <?php endwhile; ?>
    <?php else: ?>
      <p style="grid-column:1/-1; opacity:.7;">No baby girls sleepwear & underwear found.</p>
    <?php endif; ?>
  </div>
  ...
  <?php if ($totalPages > 1): ?>
    <div class="pager">
      <?php for ($i = 1; $i <= $totalPages; $i++): ?>
        ...
        <?php else: ?>
          <a href="<?= SITE_URL ?>pages/baby/bgsleep.php?page=<?= $i ?>"><?= $i ?></a>
        <?php endif; ?>
      <?php endfor; ?>
    </div>
  <?php endif; ?>

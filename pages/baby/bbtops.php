...
// Query products with sale columns
...
<head>
  <meta charset="utf-8">
  <title>Baby Boys — Tops</title>
  <link rel="stylesheet" href="<?= SITE_URL ?>css/new.css?v=<?= time() ?>">
</head>
<body>
  <div class="new-header">
    <h1 class="new-title">Baby Boys Tops</h1>
  </div>
  ...
  <div class="product-grid">
    <?php if ($result->num_rows): while ($p = $result->fetch_assoc()): ?>
      <?php
        ...
        $finalPrice = $isOnSale ? $p['sale_price'] : $p['price'];
      ?>
    ...
    <?php endwhile; else: ?>
      <p style="grid-column:1/-1; opacity:.7;">No baby boys tops found.</p>
    <?php endif; ?>
  </div>
  ...
  <?php if ($totalPages > 1): ?>
    <div class="pager">
      <?php for ($i=1; $i <= $totalPages; $i++): ?>
        ...
        <?php else: ?>
          <a href="<?= SITE_URL ?>pages/baby/bbtops.php?page=<?= $i ?>"><?= $i ?></a>
        <?php endif; ?>
      <?php endfor; ?>
    </div>
  <?php endif; ?>

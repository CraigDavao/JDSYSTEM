...
/* Query for girls products only */
$sql  = "SELECT id, name, price, image, category, created_at
         FROM products
         WHERE is_active = 1 AND category = 'girls'
...
$countSql = "SELECT COUNT(*) AS c FROM products WHERE is_active=1 AND category='girls'";
...
<body>
  ...
  <div class="new-header">
    <h1 class="new-title">New Girls</h1>
  </div>
  ...
  <?php if ($totalPages > 1): ?>
    <div class="pager">
      <?php for ($i=1; $i <= $totalPages; $i++): ?>
        ...
        <?php else: ?>
          <a href="<?= SITE_URL ?>pages/girls.php?page=<?= $i ?>"><?= $i ?></a>
        <?php endif; ?>
      <?php endfor; ?>
    </div>
  <?php endif; ?>
